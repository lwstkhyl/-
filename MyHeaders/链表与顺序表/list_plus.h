...
//BF算法找子串，返回子串第一次出现的位置 在主串中的索引（从0开始）；若没找到就返回-1
int BF_find_substring(hstring& str, hstring& substring)
{
	...
	int i = 0, j = 0; //2个指针先处于开始位置
	while (i < len_str && j < len_sub) 
	{
		if (str.ch[i] == substring.ch[j]) //如果相等就让i，j的位置往后移动
		{
			i++;
			j++;
		}
...
//KMP算法找子串，返回子串第一次出现的位置 在主串中的索引（从0开始）；若没找到就返回-1
