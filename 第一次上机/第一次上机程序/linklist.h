...
//输出循环链表存储的数据
struct link_nods* foreach_linklist_circle(link_nods* head, char seq);
//输出node节点的数据
...
//获取链表长度（不包括头节点）
int get_linklist_length(struct link_nods* header);
struct link_nods* init_linklist()
{
...
struct link_nods* foreach_linklist_circle(link_nods* head, char seq = ' ')
{
	...
	while (p->next != head && p != NULL)
	{
		printf("%d%c", p->data, seq);
		p = p->next;
	} 
	printf("\n");
	return head;
...
int get_linklist_length(link_nods* header)
{
	int len = 0;
	struct link_nods* p = header->next;
	while (p != NULL)
	{
		len++;
		p = p->next;
